...
import com.legal.sys.Iservice.IInitService;
...
public class InitService extends AbstractBaseService implements IInitService {
	...
	private Element readRootElement(String filename) {
		try {
			...
			//System.out.println(InitService.class.getClassLoader().getResource("spring_config/orgs.xml"));
			//Document d = reader.read(InitService.class.getClassLoader().getResourceAsStream("/init/"+filename));
			Document d = reader.read(InitService.class.getClassLoader().getResource("spring_config/orgs.xml"));
	...
	public void initEntityByXml(String filename) {
		Element root = readRootElement("orgs.xml");
